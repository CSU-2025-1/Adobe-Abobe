...
async def handle_upload(image: Image) -> tuple[str, str]:
    ...
    image_url, image_endpoint_url = await s3_repo.upload_to_s3(image_id, image.content, image.content_type)
    # await redis_repo.save_image_mapping(image.user_id, image_id, image_endpoint_url)
